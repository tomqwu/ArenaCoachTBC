...
// Encode once per template — same string serves both language halves.
console.log(`encoding ${templates.length} WeakAura templates...`);
const encoded = [];
for (const t of templates) {
  const s = await parser.encode(t.data, FORMAT_VERSION);
  encoded.push(s);
  console.log(`  · ${t.name}: ${s.length} bytes`);
}

// English half first, full document; then Chinese half, full document.
// Layout follows the project's `feedback_bilingual_layout` rule: each
// language is a self-contained block separated by `---` + a language
// banner, never line-by-line interleaved.
const lines = [
...
for (let i = 0; i < templates.length; i++) {
  ...
  lines.push('');
  lines.push('```');
  lines.push(encoded[i]);
  lines.push('```');
  lines.push('');
}

// Language separator + Chinese half.
lines.push('---');
lines.push('');
lines.push('# 中文 / Chinese');
lines.push('');
lines.push('# WeakAura 导入字符串 — 可直接粘贴');
lines.push('');
lines.push('> 由 `tools/export_weakauras.mjs` 自动生成。请勿手动编辑——修改源模板后重新运行导出器。');
lines.push('');
lines.push('## 安装');
lines.push('');
lines.push('1. `/wa` 打开 WeakAuras');
lines.push('2. 点击左上角 **Import**');
lines.push('3. 粘贴需要的模板字符串（`!` 开头）');
lines.push('4. 在预览对话框中点击 **Import**');
lines.push('');
lines.push('5 个模板互相独立，按需导入。导入字符串与上面英文部分相同。');
lines.push('');
lines.push('---');
lines.push('');
