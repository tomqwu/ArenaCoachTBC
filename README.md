...
2. **Active**: Mode flips to `KILL` (red) / `SWAP` (orange) / `DEFEND` (blue). The compact toast shows a punchy cue like `!! BURST !!`, `!! DANGER !!`, or `!! PINCH !!`; the big text shows the action/target; the stats row shows target HP% and kill probability; the callout row shows one utility cue; the assignments block gives up to three DBM-style actions in normal mode. `/acc verbose on` expands to full five-player assignments, chain steps, and comp/spec badges for review. If the recommendation stops refreshing, the HUD fades out instead of leaving stale text on screen.
