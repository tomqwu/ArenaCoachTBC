...
2. **Active**: Mode flips to `KILL` (red) / `SWAP` (orange) / `DEFEND` (blue). The center toast shows a punchy cue like `!! BURST !!`, `!! DANGER !!`, or `!! PINCH !!`; the left focus strip shows current targets and friendly pressure; the right cue rail shows callout icons/text; the lower assignments module gives up to three DBM-style actions in normal mode. `/acc verbose on` expands to full five-player assignments, chain steps, and comp/spec badges for review. If the recommendation stops refreshing, the HUD fades out instead of leaving stale text on screen.
