...
### UI.lua (integrated prototype-A board, v2.8.15)

The built-in HUD now follows the agreed prototype-A layout as one compact texture-backed board. The board is 460x168 and keeps the left focus panel, center action panel, right cue panel, and lower assignment panel together when dragged. The shell uses a light translucent background, a subtle top drag strip, a small grip marker, fixed internal dividers, and shadowed/highlighted text so users can see exactly where the movable board starts and ends without covering the fight with a dark plate. This replaces the v2.8.13/v2.8.14 dependence on detached satellite frames, which could be left elsewhere when the user moved the main frame. The left panel summarizes the current kill/swap focus and lowest friendly pressure. The right panel shows callout icons/text such as burst, purge, HoJ, peel, or dispel cues. The lower assignment panel shows up to three player jobs in normal mode, while `/acc verbose on` keeps the full five-player assignment list and diagnostic detail for reviews. Waiting/pre-gate states render structural placeholders in all zones so the HUD does not collapse back into floating text before live combat data exists. `/acc unlock` drags the integrated board; `/acc lock` locks it.

内置 HUD 现在按已确认的 A 方案做成一个紧凑的贴图背景面板。面板为 460x168，拖动时会把左侧焦点、中间行动、右侧提示、下方分工四个区域一起移动。这样不再依赖 v2.8.13/v2.8.14 的分离卫星框体，避免主框体移动后其它模块留在别处。左侧区域显示当前击杀/换火目标和最低血量友方压力；右侧区域显示爆发、驱散、制裁、保护、解控等图标/文字提示；下方区域普通模式最多显示三条玩家任务。等待/开门前状态会在所有区域显示占位结构，避免在没有实时战斗数据时退回成纯浮动文字。`/acc verbose on` 保留完整五人分工和诊断细节。`/acc unlock` 拖动整块面板，`/acc lock` 锁定。
