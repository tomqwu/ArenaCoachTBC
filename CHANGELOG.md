...
- **Advice + Facts HUD stayed on screen in a city right after an arena (user report: full KILL callout in Shattrath after a 5v5).** Two causes:
  - `DetectPvPContext` checked `IsActiveBattlefieldArena()` before `GetInstanceInfo()`. That battlefield flag stays true for a beat after a match while the client ports you out, so whichever zone event fired as you landed in the city cached `pvpContext = "arena"` and the auto-hide gate never fired. `GetInstanceInfo()` is now authoritative — a resolved non-PvP instance type (`none`/`party`/`raid`) drops the context to `world_idle`/`none`, and the lingering arena flag is trusted only as a fallback when the instance type is still unresolved (mid-load).
  - The Facts HUD ticker repainted from the last state snapshot without re-checking context, so it could keep a populated panel alive even after Core stopped evaluating. It now mirrors the same context gate on each repaint and self-hides within one tick once the context is idle.
- **Facts HUD layout broke on long names + Chinese spell names.** Player names like `Roadtoisekai` were wrapping into the trinket cell; defensive cell text like `寒冰屏障 5m` / `暗影斗篷 2m` was overflowing under the next row (Chinese characters render wider in the small font than the v2.10 76 px column allowed). Two root causes addressed:
  ...
  - Column widths reworked: Name 110 → 130, Trinket 42 → 56, Defensive 76 → 130 (fits Chinese spell names + countdown), Interrupt 26 → 110 (fits `Counterspell 24`), DR 42 → 70 (fits two active badges like `S:1/4 F:IMM`). Column x-offsets and the frame width are now **derived** from the widths (`FH.COL` / `FH.FRAME_WIDTH`, landing at 560), so widening one cell can't silently desynchronise the rest of the layout.
- **Interrupt cell always said "KICK" regardless of which interrupt was observed.** It now shows the actual `GetSpellInfo(spellID)` result — `Counterspell 24`, `Earth Shock 6`, `Pummel 10`, ... — via the same formatter as the defensive cell (one shared `formatSpellCell`, so the two columns can't drift again). The fallback label when the spell name isn't cached is the generic `INT` (locale key `FACTS_KICK_LABEL`), and the formatter treats `GetSpellInfo` returning `""` the same as `nil` — an empty string is truthy in Lua, so a bare `or` fallback never fired.
- **"Save Tremor for fear" / Grounding+Tremor advice could fire against comps with no fear.** The fear-threat gate is now purely structural — a Tremor callout requires a *living* PRIEST / WARLOCK / WARRIOR on the enemy team. The old gate also accepted profile-driven callouts as evidence of a fear threat, which let `CALL_SAVE_TREMOR_HOJ` legitimise itself (circular). The shaman DEFEND label likewise splits: `ACTION_SHAMAN_DEFEND` (Grounding/Tremor) only with a fear-class enemy alive, otherwise the new `ACTION_SHAMAN_DEFEND_GROUNDING` (Grounding-only wording).
...
- `FACTS_DEFENSIVES` / `FACTS_INTERRUPTS` display lists and a `CLASS_SELF_IMMUNITY` map in `Data/Spells.lua`. **Every rank** of multi-rank spells is registered (Kick r1-r5 incl. 38768, Pummel r1-r2, Earth Shock r1-r8, Counterspell cast ID 2139, Divine Shield r1-r2 incl. 1020, BoP r1-r3, Evasion r1-r2, Vanish r1-r3, Ice Block alt 45438) — CLEU carries the rank actually cast and the trackers match IDs exactly, so without rank coverage a level-70 kick or bubble was invisible.
...
- **`major_defensive_down` scoring weight now fires — narrowly.** It read `enemy.majorDefensiveDown`, which no production code ever set. The bonus now derives from observed cooldowns of the target's **own class immunity** (`CLASS_SELF_IMMUNITY`: Ice Block / Divine Shield / Cloak of Shadows) seen >= 15s from ready, and never fires while the target is actively immune. Cross-target spells like BoP are excluded — a paladin who BoP'd a teammate has not burned their own escape. The broad `kill_defensive_soon` penalty (< 15s on ANY observed immunity) is unchanged.
...
### Tests
- Added arena-shaped regressions for high HP, immunity, missing MS/Wound, stun DR immunity, no-Shaman rosters, and a positive kill-window fixture with expected `duration`/`expiresAt` on `BURST_NOW` and Bloodlust signals.
...
### Tests
- Extended WeakAuraBridge specs for active bar signals, owner/target metadata, rejected/blocked summaries, no-state safety, and schema-version reporting.
...
### Tests
- Added a tiny SavedVariables replay fixture plus a committed golden report so strategy drift is caught by the Lua test suite before release.
...
### Tests
- Added Core regressions for rejected reason trace output, own-capability snapshots, player action summaries, trace dump readability, and bug report redaction of strategy diagnostics.
