...
H.it(g, "CreateFrame builds prototype-A module set", function()
    ...
    H.assertTrue((f._width or 999) <= 560, "main board width should stay compact")
    H.assertTrue((f._height or 999) <= 220, "main board height should stay compact")
    ...
    H.assertEq(f._maxResize[1], 820)
    H.assertEq(f._maxResize[2], 360)
    H.assertEq(f.centerPanel._point[5], -32)
    H.assertEq(f.centerPanel._point[4], 147)
    H.assertEq(f.rightPanel._point[4], 417)
    H.assertEq(f._accCenterSubLines, 2)
    H.assertEq(f._accCueLines, 5)
    H.assertEq(f._accAssignLines, 4)
