...
H.it(g, "CreateFrame builds prototype-A module set", function()
    ...
    H.assertNotNil(f.modeAccent)
    H.assertNotNil(f.metaText)
    H.assertNotNil(f.healthBarBg)
    H.assertNotNil(f.healthBarFill)
    ...
    H.assertTrue((f._width or 999) <= 420, "vertical board should stay narrow enough to park beside arena frames")
    H.assertTrue((f._height or 999) <= 420, "vertical board should stay short enough to avoid action bars")
    ...
    H.assertTrue((UI.assignFrame._height or 999) <= 160, "assignment module should be vertical but still compact")
    ...
    H.assertEq(f.leftPanel._width, 364)
    H.assertEq(f.centerPanel._width, 364)
    H.assertEq(f.rightPanel._width, 364)
    H.assertEq(f.assignPanel._width, 364)
    H.assertEq(f.leftPanel._height, 56)
    H.assertEq(f.centerPanel._height, 94)
    H.assertEq(f.rightPanel._height, 75)
    H.assertEq(f.assignPanel._height, 91)
    ...
    H.assertEq(f._minResize[1], 340)
    H.assertEq(f._minResize[2], 320)
    H.assertEq(f._maxResize[1], 620)
    H.assertEq(f._maxResize[2], 560)
    H.assertEq(f.centerPanel._point[5], -32)
    H.assertEq(f.centerPanel._point[4], 8)
    H.assertEq(f.rightPanel._point[4], 8)
    H.assertEq(f.rightPanel._point[5], -198)
    H.assertEq(f._accCenterSubLines, 2)
    H.assertEq(f._accCueLines, 5)
    H.assertEq(f._accAssignLines, 5)
    H.assertNotNil(f.topTick1)
    ...
    H.assertTrue((f._accBg._color and f._accBg._color[4] or 0) >= 0.24,
        "main board obsidian field should still hold text together")
    H.assertTrue((f._accBg._color and f._accBg._color[4] or 1) <= 0.32,
        "main board obsidian field should preserve more world visibility")
    H.assertTrue((f._accBg._color and f._accBg._color[1] or 0) > 0,
        "main board background should be warm obsidian, not flat black")
    H.assertTrue((f.leftPanel._accBg._color and f.leftPanel._accBg._color[4] or 0) >= 0.20,
        "module reading plates should remain visible on busy scenes")
...
H.it(g, "layout keeps center copy inside its section on wide boards", function()
    ...
    UI.railFrame = nil
    local f = UI:CreateFrame()
    H.assertTrue(f.centerPanel._width >= 340, "wide boards should give action text real breathing room")
    H.assertTrue(f.leftPanel._width >= 170, "wide boards should widen the focus section")
    H.assertTrue(f.rightPanel._width >= 150, "wide boards should widen the cue section")
...
H.it(g, "CreateFrame restores and saves resized prototype-A board dimensions", function()
    ...
    H.assertEq(f._height, 320)
    H.assertTrue(f.centerPanel._width > 480, "vertical action panel should span the resized board width")
    H.assertTrue(f.assignPanel._width >= f.centerPanel._width,
        "player-info panel should stay aligned under the action stack")
    H.assertTrue(f.leftPanel._point[5] < f.centerPanel._point[5],
        "focus section should stack below the action section")
    H.assertTrue(f.rightPanel._point[5] < f.leftPanel._point[5],
        "strategy section should stack below the focus section")
    H.assertTrue(f.assignPanel._point[5] < f.rightPanel._point[5],
        "assignment section should stack below the strategy section")
    ...
    H.assertEq(_G.ArenaCoachTBCDB.frame.height, 320)
    H.assertTrue(f.centerPanel._width > 520, "layout should recompute after resize")
end)
